...
const OrderDetails = () => {
  ...
  const { line1, line2, city, state, country } = shipping || {};
    ...
    <div className={styles.table}>
      ...
      <div className={styles['details-wrapper']}>
              ...
              return (
                <React.Fragment key={id}>
                  <tr>
                    ...
                    <td>
                      <p style={{color: '#000', marginBottom: '10px', fontWeight: '100'}}>
                        <b>{name}</b>
                      </p>
                      {imageErrors[index] ? (
                        <p>Image not available (Product might have been deleted)</p>
                      ) : (
                        <img
                          src={imageURL}
                          alt={name}
                    ...
                    <td className={`${styles.icons} ${styles.showOnMobile}`}>
                      <button onClick={(e) => handlePreview(e, index)} title="preview-button">
                        {!showPreview[index] ? (
                          <ChevronRightIcon color="#007AFF" size={20} title="right-arrow-icon" />
                        ) : (
                          <ChevronDownIcon color="#007AFF" size={20} title="down-arrow-icon" />
                        )}
                      </button>
                    </td>
                  </tr>
                    ...
                    <tr className={styles['additional-row']}>
                      <td colSpan="6">
                        <div className={styles['toggle-content']}>
                          ...
                          <p>
                            <b>Cart Quantity:</b> {cartQuantity}
                          </p>
                          <p>
                            <b>Total Price:</b> {formatNaira(price * cartQuantity)}
                          </p>
                        </div>
                      </td>
                    </tr>
        ...
        <div className={styles['summary-wrapper']}>
          ...
          <p><b>Order Amount:</b> {formatNaira(order.orderAmount)}</p>
          <p><b>Order Status:</b> {order.orderStatus}</p>
          <p><b>Shipping Address:</b> {line1 || 'N/A'}, {line2 || 'N/A'}, {city || 'N/A'}</p>
